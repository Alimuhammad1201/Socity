                    ...
                    <div class="card border-top  border-white">
                        <div class="card-body p-5">
                            <div class="card-title d-flex align-items-center">
                                ...
                                <h5 class="mb-0 text-white">Resident Document Add</h5>
                            </div>
                            <hr>
                            ...
                            @endif
                            <form class="row g-3" action="{{ route('resident_document.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-control" id="document_type" name="document_type">
                                        ...
                                        <option value="pending" {{ old('document_type')=='pending' ? 'selected' : '' }}>Pending
                                        </option>
                                        <option value="confirmed" {{ old('document_type')=='confirmed' ? 'selected' : '' }}>Confirmed
                                        </option>
                                        <option value="cancelled" {{ old('document_type')=='cancelled' ? 'selected' : '' }}>Cancelled
                                        </option>
                                    </select>
                                    @error('document_type')
                                    <div class="text-danger">{{ $message }}</div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="file" class="form-control" id="document_path" name="document_path"
                                           placeholder="Monthly Rent" value="{{ old('document_path') }}">
                                    @error('document_path')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
