                    ...
                    <div class="card border-top  border-white">
                        <div class="card-body p-5">
                            <div class="card-title d-flex align-items-center">
                                ...
                                <h5 class="mb-0 text-white">Edit Admin Building</h5>
                            </div>
                            <hr>
                            ...
                            @endif
                            <form class="row g-3" action="{{ route('admin-building.update',$building_admin->id) }}" method="post">
                                @csrf
                                <input type="hidden" name="id" id="id" value="{{ $building_admin->id }}">
                                <div class="col-md-6">
                                    ...
                                    <select class="form-control" id="assign_building" name="assign_building">
                                        ...
                                        @foreach($assign_building as $row)
                                            <option value="{{ $row->id }}"{{ $row->id == $building_admin->assign_building ? 'selected' : '' }}>{{ $row->building_name }}</option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-control" id="feature" name="feature[]" multiple>
                                        ...
                                        @if(isset($package_feature) && is_array($package_feature))
                                            @foreach($package_feature as $feature)
                                                <option value="{{ $feature }}"{{ $row->id == $building_admin->feature ? 'selected' : '' }}>{{ $feature }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('feature')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" id="name" name="name"
                                           placeholder="Name" value="{{ old('name', $building_admin->name) }}">
                                    @error('name')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="email" class="form-control" id="email" name="email"
                                           placeholder="Email" value="{{ old('email', $building_admin->email) }}">
                                    @error('email')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
